...
if (isset($_POST['projectUpdate']) && isset($_GET['projectId'])) {
    ...
    try {
        ...
        if ($stmt = mysqli_prepare($conn, $sql)) {
            ...
            header("Location:ongoing_projects.php?status=success&message=project updated successfully");
            exit();
        } else {
            throw new Exception(mysqli_error($conn));
        }
    } catch (Exception $e) {
        header("Location:ongoing_projects.php?status=unsuccess&message=project updated unsuccessfully " . $e->getMessage());
        exit();
    }

} else {
    header("Location: ongoing_projects.php?status=unsuccess&message=project updated unsuccessfully");
    exit();
}
?>
